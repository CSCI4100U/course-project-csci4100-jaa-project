...
class VerticalChart extends StatefulWidget {
  ...
  VerticalChart({
    ...
    this.chartButtonIcon = const Icon(Icons.align_vertical_bottom),
  }) : super(key: key);
...
class _VerticalChartState extends State<VerticalChart> {
  ...
  Widget build(BuildContext context) {
    ...
    /// adding chart data
    priceData = ChartWidgets()
        .addChartData(widget.dataItems, Colors.blue, columnNames[2]);
    ratingData = ChartWidgets()
        .addChartData(widget.dataItems, Colors.red, columnNames[3]);
    capacityData = ChartWidgets()
        .addChartData(widget.dataItems, Colors.orange, columnNames[4]);
    ratingValuePerPrice = ChartWidgets()
        .addChartData(widget.dataItems, Colors.green, columnNames[5]);
    capacityPerPriceData = ChartWidgets()
        .addChartData(widget.dataItems, Colors.purple, columnNames[6]);

    List<charts.Series<ChartData, String>> series = ChartWidgets()
        .fillChartSeries(priceData, ratingData, ratingValuePerPrice,
            capacityData, capacityPerPriceData);

    return Scaffold(
      appBar: AppBar(
        title: Text(
          FlutterI18n.translate(context, "charts.vertical_bar.title"),
      ...
      body: widget.dataItems!.isNotEmpty
          ? Zoom(
