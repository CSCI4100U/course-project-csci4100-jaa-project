...
class _BodyState extends State<Body> {
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<Event>> getEvents() async {
      List<Event> events = await EventModel().getAllEvents();
      return events;
    }
      ...
      child: FutureBuilder(
                      ...
                      for (var event in snapshot.data as List<Event>)
                        ...
                        return GestureDetector(
                          onTap: () {
                            ...
                            selectedIndex = snapshot.data!.indexOf(event);
                            currentLocation = LatLng(event.location!.latitude,
                                event.location!.longitude);
                          ...
                          child: AnimatedScale(
                            duration: const Duration(milliseconds: 500),
                            scale: selectedIndex == snapshot.data!.indexOf(event) ? 1 : 0.7,
                            child: AnimatedOpacity(
                              duration: const Duration(milliseconds: 500),
                              opacity: selectedIndex == snapshot.data!.indexOf(event) ? 1 : 0.5,
                              child: const Icon(
                                Icons.location_on,
                                color: Colors.red,
                                size: 40,
        ...
        Positioned(
            ...
            child: PageView.builder(
              ...
              onPageChanged: (value) {
                selectedIndex = value;
                currentLocation = LatLng(snapshot.data![value].location!.latitude,
